...
編碼位置通常使用 16 進制的格式，位元捕 0 至少 4 位，使用 `U+` 當作前綴字。
編碼可能的範圍從 `U+0000` 到 `U+10FFFF` 超過 110 萬個符號。為了確保事情的組織性，Unicode 把這個範圍的編碼區分成 17 個區段，各自由 65536 個編碼組成。
如果你曾經看過 Wiki 百科上的翻譯他翻成`平面`，由 17 個平面組成。
...
補充平面非常好辨別: 如果某個字符需要超過 4 位元的 16 進制來表示那它就屬於補充平面。
...
這個東西術語叫做 16 進制的跳脫序列。由 2 個位元 16 進制組成代表一個編碼位置。舉例來說 `\x41` 代表 `U+0041`。
...
這種格式被稱作萬國碼跳脫序列，算了還是記英文吧 Unicode escape squences 由精準的 4 位元 16 進制代碼組成表達編碼位置，舉例來說: `\u2661` 表示 `U+2661` 這種跳脫序列可以用來表示 `U+0000` 到 `U+FFFF` 範圍的萬國碼 Unicode 等於是整個基本多文種平面(BMP) 

那麼..其他平面呢? 我們需要大於 4 位元來表示其他編碼位置，那我們要如何跳脫 Escape 它們?

在 ES6 裡面這將會變得相對簡單，因為引進了新類型的跳脫序列: `Unicode code point escapes`
...
在這種情況下每一個跳脫序列代表一半的編碼位置，2 個代理編碼組成一個編碼的位置。
...
透過這種代理編碼的機制所有補充平面的編碼位置(U+010000 - U+10FFFF) 都可以使用。不過使用單一跳脫字元來表示 BMP 裡面的字，兩個跳脫字元組成代理編碼來處理剩下補充平面的字很容易讓人搞混，造成很多惱人的後果。
...
上面這個例子 `length` 剛好是字元的數量，說有 1 個文字這很合理。那如果我們使用跳脫字元來呈現一個文字呢?
...
在內部 JavaScript 把補充平面的字符視為代理編碼(就是用兩個跳脫字元表示一個字)且把這個代理編碼視為 2 個分開的字元。如果您在 ES5 兼容的瀏覽器輸出您會看到他把他視為兩個跳脫字元 length 為 2 ，人們對於字面上只顯示一個字的理解上會產生困惑。


# 計算補充平面裡的文字
回到剛剛的問題，那我們如何計算 JS 字串中有幾個字?
這個小技巧針對`代理編碼`，當我們認出這兩個跳脫字元會組成一個字的時候只計算一次
...
這個標準化 `normalize` 方法是內建 String.prototype 的方法，他會執行[Unicode normalization](http://unicode.org/reports/tr15/)根據規則找出那些看起來一樣卻不同編碼的差異，如果找到那種`兩個編碼組成的字`卻長得跟`另一單一編碼位置`一樣的字，它會把它轉成單一的那種編碼
...
幸運的是有人解決了這個問題，只要透過[Esrever](https://mths.be/esrever)
...
除了陣列 `reverse()` 的行為外，這種問題也影響到騎到字串的方法。
...
>C語言、Shell、Python、Java語言及其他相近的語言使用字首「0x」，例如「0x5A3」。

>在HTML，十六進制可以用「x」，例如 &#x0041; 會等於 「A」。
...
## 從字串取得字的編碼位置
類似於上面的狀況，如果您使用 `Strint.prototype.charCodeAt(index)` 來檢索字串中第一個字的編碼位置，您一樣會取得代理編碼 2 個編碼中的第一個編碼
...
## 遍歷字串中的每個字
假設您想要用迴圈遍歷(好文鄒鄒就是一個一個拿)一個字串，分別對每個字做點處理。
...
## 比對匹配代碼位置(Code Point)與 Unicode 標量值(Unicode Scalar Values)
...
這個正規式可以匹配基本多文種平面，補充平面的字，甚至只有一半的代理編碼。

只寫一半的代理編碼在技術上是可行的，但因為他們不會對應到任何實際的字所以應該避免。
Unicode 標量值(Unicode Scalar Values)指的是所有的編碼位置但扣掉那些用來組合代理編碼的`編碼位置` 
...
注意到 `.` 仍然不會匹配到換行字元，當設定了 `u` flag 之後在兼容環境中等於是使用下面的程式碼
...
# 結論
一般處理單一語系的開發者其實不太容易注意到這些問題。這也是在學習 Babel 的過程中為了理解為什麼要特別把 Unicode 再拉出來而寫的學習筆記。
